...
class AttentionVisualizer:
    ...
    def translate(self, split):
        ...
        logger.info('Starting attention exporting')
        ...
        with torch.no_grad():
            results = translator.run_all()
        
        attn = self.model.get_attention_weights()['decoder']
        
        
        import numpy as np
        for i in range(len(attn)):
            txt_attn = attn[i]['cross']['txt'].cpu().detach().numpy()
            img_attn = attn[i]['cross']['img'].cpu().detach().numpy()
            
            txt_attn.dump(self.output+"_cor.layer{}.txt_attn.npy".format(i))
            img_attn.dump(self.output+"_cor.layer{}.img_attn.npy".format(i))
            
        with open(self.output+".output", 'w') as f:
            for line in results:
                f.write(line[0] + '\n')
