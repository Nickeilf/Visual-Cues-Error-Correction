...
class LabelSmoothingCELoss(nn.Module):
    ...
    """
    def __init__(self, smoothing=0.1, ignore_index=0, reduce=True):
        """Adapted from fairseq implementation
        https://github.com/pytorch/fairseq/blob/master/fairseq/criterions/label_smoothed_cross_entropy.py#L33
        """
    ...
    def forward(self, x, target):
        logprobs = x
        if target.dim() == logprobs.dim() - 1:
            target = target.unsqueeze(-1)

        nll_loss = -logprobs.gather(dim=-1, index=target)
        smooth_loss = -logprobs.sum(dim=-1, keepdim=True)
        ...
        eps_i = self.smoothing / (logprobs.size(-1) - 1)
...
class ScaleNorm(nn.Module):
    ...
    def __init__(self, scale, eps=1e-5):
        ...
        self.scale = torch.tensor(scale, requires_grad=True).to(DEVICE)
        self.eps = eps
