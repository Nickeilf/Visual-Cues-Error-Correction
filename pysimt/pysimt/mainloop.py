...
class MainLoop:
        ...
        if 'loss' in out:
            ...
            for idx, tid in enumerate(out):
                if idx == 0:
                    ...
                    self.loss_meter.update(out[tid]['loss'], out[tid]['n_items'], task2=True)
            self.loss_meter.store_multi_loss(out)
            ...
            loss = sum([out[k]['loss'] / out[k]['n_items'] for k in out])
    ...
    def train_epoch(self):
        ...
        for batch in self.train_iterator:
            ...
            if self.monitor.uctr % self.disp_freq == 0:
                ...
                if self.loss_meter.loss_dict:
                    msg = "Epoch {} - update {:10d} => {} loss: {:>7.3f} | {} loss: {:>7.3f} | Total loss: {:>7.3f}".format(
                        self.monitor.ectr, self.monitor.uctr,
                        self.loss_meter.loss_dict['task'][0], self.loss_meter.loss_dict['loss'][0],
                        self.loss_meter.loss_dict['task'][1], self.loss_meter.loss_dict['loss'][1],
                        self.loss_meter.loss_dict['loss'][0] + self.loss_meter.loss_dict['loss'][1]
                    )
                else:
        ...
        # Compute epoch loss
        if self.loss_meter.loss_dict:
            mt_loss, cor_loss = self.loss_meter.get_separate()
            self.monitor.train_loss.append(mt_loss)
            self.print("--> Epoch {} finished with mean loss: MT loss {:.5f} | COR loss {:.5f}".format(
                self.monitor.ectr, mt_loss, cor_loss))
        else:
    ...
    def do_validation(self):
        ...
        if self.monitor.beam_metrics:
            ...
            self.print(f'Performing {translator_name} with size {beam_size} (args: {tr_args})')
            beam_time = time.time()
            # Use greedy search
