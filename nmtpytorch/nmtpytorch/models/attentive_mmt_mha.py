...
from ..layers import ConditionalMMDecoder, TextEncoder, FF, TextGating, MultiHeadAttention
from .nmt import NMT

logger = logging.getLogger('nmtpytorch')


class SelfAttentiveMMT(NMT):
    ...
    def setup(self, is_train=True):
        ...
        self.mha = MultiHeadAttention(model_dim=txt_ctx_size,
                                        n_heads=self.opts.model['n_heads'],
                                        dropout=self.opts.model['dropout_ctx'],
                                        with_image=True)
    ...
    def encode(self, batch, **kwargs):
        ...
        # SxBxC
        text_hidden = text_encoding[0]

        seq_len = text_hidden.shape[0]
        batch_size = text_hidden.shape[1]

        # (S+1)xBxC
        ctx = torch.cat((feats, text_hidden), 0)
        
        text_hidden = self.mha((ctx, ctx, ctx))
